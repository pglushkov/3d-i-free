...
void draw_geometry(GLuint gl_program, GLuint vert_buffer_id, GLuint vert_order_buffer_id, size_t vert_order_array_len,
                   ...
                   float offset_x, float offset_y, float offset_z)
{
        ...
        bind_uniforms(gl_program, angle_x, angle_y, angle_z, offset_x, offset_y, offset_z); // passing some custom uniform data to the shader
...
void bind_uniforms(GLuint gl_program, float angle_x, float angle_y, float angle_z,
                   float offset_x, float offset_y, float offset_z)
{
    // setting necessary uniforms

    GLint uniform_handle = glGetUniformLocation(gl_program, "in_angle_x");
    if (uniform_handle != -1) {
        glUniform1f(uniform_handle, angle_x);
    } else {
        printf("uniform %s fuck-up ... \n", "in_angle_x");
        throw std::runtime_error("ERROR in bind_uniforms : Could not locate a uniform!");
    }

    uniform_handle = glGetUniformLocation(gl_program, "in_angle_y");
    if (uniform_handle != -1) {
        glUniform1f(uniform_handle, angle_y);
    } else {
        printf("uniform %s fuck-up ... \n", "in_angle_y");
        throw std::runtime_error("ERROR in bind_uniforms : Could not locate a uniform!");
    }

    uniform_handle = glGetUniformLocation(gl_program, "in_angle_z");
    if (uniform_handle != -1) {
        glUniform1f(uniform_handle, angle_z);
    } else {
        printf("uniform %s fuck-up ... \n", "in_angle_z");
        throw std::runtime_error("ERROR in bind_uniforms : Could not locate a uniform!");
    }

    uniform_handle = glGetUniformLocation(gl_program, "in_x_offset");
    if (uniform_handle != -1) {
        glUniform1f(uniform_handle, offset_x);
    } else {
        printf("uniform %s fuck-up ... \n", "in__offset_x");
        throw std::runtime_error("ERROR in bind_uniforms : Could not locate a uniform!");
    }

    uniform_handle = glGetUniformLocation(gl_program, "in_y_offset");
    if (uniform_handle != -1) {
        glUniform1f(uniform_handle, offset_y);
    } else {
        printf("uniform %s fuck-up ... \n", "in__offset_y");
        throw std::runtime_error("ERROR in bind_uniforms : Could not locate a uniform!");
    }

    uniform_handle = glGetUniformLocation(gl_program, "in_z_offset");
    if (uniform_handle != -1) {
        glUniform1f(uniform_handle, offset_z);
    } else {
        printf("uniform %s fuck-up ... \n", "in__offset_z");
        throw std::runtime_error("ERROR in bind_uniforms : Could not locate a uniform!");
    }
